...
def data_generator(text_file, batch_size):

    text_sequences = []
    sequences = {}
    count,count_lines = 1,1
    text = ''
	# keep looping until we reach our batch size
    while count_lines < batch_size:
        text += get_sentence() + ' '
...
#    seq_len = train_inputs.shape[1]
    ...
    return train_inputs,train_targets,vocabulary_size,tokenizer
...
#Fitting the model
model.fit_generator(train_inputs,train_targets,steps_per_epoch=vocabulary_size // batch_size,epochs=num_epochs,shuffle=True,verbose=1,callbacks=[checkpoint])
